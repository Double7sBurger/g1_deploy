...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="newton",
        help="Joint layout the checkpoint was trained against. ``physx`` and ``newton`` are two"
        " enumerations of the same superseded 37-joint USD -- PhysX breadth first, Newton depth"
        " first -- and the wrong choice silently drives the wrong motors. ``g1_29dof`` is the"
        " current robot description: a different robot with 43 joints, so a checkpoint trained on"
        " it takes a 690-element observation where the others take 600 and simply will not load"
        " under the wrong one.",
    )
